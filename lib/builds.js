...
Builds.prototype._getBuildByIdCallback = function( error, res, body, callback ) {
  ...
  callback( null, response, buildId );
};
...
Builds.prototype._getBuildsByIds = function( buildIds, callback ) {
  ...
  buildIds.forEach( function( id ) {
    this._getBuildById( id, function( error, build ) {
 ...
 * @tested
 */
Builds.prototype._getBuildsByIdsLoopCallback  = function( error, build, buildIds, responseArray, eventEmitter, callback ) {
  if ( error ) {
    callback( error );
...
/**
 * Function to call callback finally when
 * all builds are fetched
 *
 * @param  {Array}   responseArray array with particular builds
 * @param  {Array}   buildIds      original build ids
 * @param  {Function} callback     callback
 *
 * @tested
 */
Builds.prototype._sendBuilds = function( responseArray, buildIds, callback ) {
  if ( buildIds.length === responseArray.length ) {
